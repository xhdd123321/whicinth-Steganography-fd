...
const decodeHandle = async () => {
  loading.value = true
  try {
    const res = await stegApi.decodeIntelligent(formData.value)
    if (res.code === 0) {
      clearResult()
      enterGoCD()
      if (res.data?.url) {
...
<template>
  <div class="card-content">
    <a-page-header title="Decode" subtitle="intelij" :show-back="false">
      <template #extra>
    ...
    <a-card class="step" :loading="loading">
      ...
      <a-image v-if="resultUrl !== ''" width="70%" class="result-image" :src="resultUrl" />
